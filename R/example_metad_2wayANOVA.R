...
# Simulate 3x3 repeated measures design: 
# Task (Perception, Memory, Cognition) x Order (Forward, Simultaneous, Reverse)
# Task: (P,P,P,M,M,M,C,C,C) - Order: (F,S,R,F,S,R,F,S,R)

nR_S1 = list(
  list(c(10.,5.,3.,2.), c(4.,7.,6.,3.), c(4.,6.,8.,2.), 
       c(9.,4.,4.,3.), c(3.,6.,7.,4.), c(2.,8.,7.,3.), 
       c(11.,4.,2.,3.), c(5.,6.,4.,5.), c(5.,5.,6.,4.)),
  list(c(0.,10.,6.,4.), c(1.,9.,5.,5.), c(3.,3.,11.,3.), 
       c(4.,6.,5.,5.), c(1.,12.,4.,3.), c(3.,6.,9.,2.), 
       c(3.,12.,2.,3.), c(3.,7.,7.,3.), c(2.,8.,5.,5.))
  )
nR_S2 = list(
    list(c(2.,5.,3.,10.), c(3.,7.,6.,4.), c(2.,6.,7.,5.), 
         c(3.,4.,4.,9.), c(2.,7.,6.,5.), c(1.,8.,7.,4.), 
         c(3.,4.,3.,10.), c(2.,6.,4.,8.), c(1.,5.,6.,8.)),
    list(c(6.,10.,0.,4.), c(1.,4.,5.,10.), c(1.,4.,10.,5.), 
         c(2.,6.,5.,7.), c(1.,3.,5.,11.), c(1.,6.,8.,5.), 
         c(1.,2.,12.,5.), c(1.,6.,8.,5.), c(2.,5.,7.,6.))
  )
...
PlotCondition2 <- mcmc.sample %>%
  ...
  ggplot(aes(value)) +
  geom_histogram(fill = "blue", colour = "grey", alpha = 0.5, bins = 100) +
  geom_vline(xintercept = stat$mean[stat$name == "muBd_Condition2"],linetype="dashed", size = 1.) +
  geom_vline(xintercept = 0, size = 1.5, color='red') +
  geom_segment(aes(x = HDI$lower[HDI$name == "muBd_Condition2"], y = 50, xend = HDI$upper[HDI$name == "muBd_Condition2"], yend = 50), colour = "white", size = 2.5) +
  ylab("Sample count") +
...
PlotInteraction <- mcmc.sample %>%
  ...
  ggplot(aes(value)) +
  geom_histogram(fill = "blue", colour = "grey", alpha = 0.5, bins = 100) +
  geom_vline(xintercept = stat$mean[stat$name == "muBd_interaction"],linetype="dashed", size = 1.) +
  geom_vline(xintercept = 0, size = 1.5, color='red') +
  geom_segment(aes(x = HDI$lower[HDI$name == "muBd_interaction"], y = 50, xend = HDI$upper[HDI$name == "muBd_interaction"], yend = 50), colour = "white", size = 2.5) +
  ylab("Sample count") +
